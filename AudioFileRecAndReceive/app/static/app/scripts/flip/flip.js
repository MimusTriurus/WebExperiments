    ...
    Plugin.prototype = {
        ...
        contentHTML: function(data){
            ...
            for (var i=0; i<data.length; i++){
                ...
                if(data[i].displayValue){
                    ...
                    if(data[i].rmType === "DV_DATE" || data[i].rmType === "DV_TIME" || data[i].rmType === "DV_DATE_TIME"){
                        value = this._formatDate(value, true);
                    }

                    if(data[i].rmType === "DV_DURATION"){
                        const period = Iso8601Period.valueOf(value);
                        value = period.years > 0 ? period.years + "y" : "";
                        if (period.months > 0) value +=  " " + period.months + "m";
                    }
        ...
        getDemographics: function(){
            ...
            return $.ajax({
                    ...
                    type: 'GET',
                    headers: {
                        "Authorization": "Basic " + btoa (self.options.username + ":" + self.options.password)
                    },
